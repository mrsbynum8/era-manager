...
export async function POST(req: Request) {
    try {
        ...
        if (type === "niches") {
            const unassigned = db.getUnassignedDesigns().slice(0, 50);
            dataContext = `Unassigned Designs (sample): ${unassigned.map(d => d.name).join(", ")}`;
            ...
            const unassigned = db.getUnassignedDesigns(); // Get all to search properly
            const nicheName = context;

            // Simple heuristic search in JS to find candidates for the prompt
            const candidates = unassigned
                .filter(d => d.name.toLowerCase().includes(nicheName.toLowerCase()) || d.cleanName.toLowerCase().includes(nicheName.toLowerCase()))
                .slice(0, 30);
            ...
            if (targetNiche) {
                const existingDesigns = db.getDesigns().filter(d => d.nicheIds.includes(targetNiche.id));
            ...
            prompt = `Based on the existing designs for the "${nicheName}" niche (if any), suggest 10 NEW "In My [X] Era" design ideas that are missing and would sell well.`;
        }
